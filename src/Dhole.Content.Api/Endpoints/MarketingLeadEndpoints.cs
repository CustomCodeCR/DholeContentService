...
public static class MarketingLeadEndpoints
{
    ...
    {
        var group = app.MapGroup("/api/content/leads")
            .WithTags("Marketing Leads")
        ...
        group.MapPost("/", async (
                CreateMarketingLeadRequest request,
                ICommandDispatcher dispatcher,
                HttpContext context,
                CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(new CreateMarketingLeadCommand(
                request.SiteKey, request.FirstName, request.LastName, request.Email, request.Phone, request.Company,
                request.JobTitle, request.Country, request.Source, request.Status, request.OwnerUserId,
        ...
        group.MapPut("/{id:guid}", async (
                ...
                HttpContext context,
                CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(new UpdateMarketingLeadCommand(
                id, request.SiteKey, request.FirstName, request.LastName, request.Email, request.Phone, request.Company,
                request.JobTitle, request.Country, request.Source, request.Status, request.OwnerUserId,
                request.LastTouchAtUtc, context.GetCurrentUserId()), ct), context))
            .RequireScope(ContentScopeNames.Edit);

        group.MapPost("/{id:guid}/touch", async (
                Guid id,
                TouchMarketingLeadRequest request,
                ICommandDispatcher dispatcher,
                HttpContext context,
                CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(
                new TouchMarketingLeadCommand(id, request.Source, context.GetCurrentUserId()), ct), context))
            .RequireScope(ContentScopeNames.Edit);
        ...
        group.MapDelete("/{id:guid}", async (
                ...
                CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(
                new DeleteMarketingLeadCommand(id, context.GetCurrentUserId()), ct), context))
            .RequireScope(ContentScopeNames.Edit);
