...
public static class MarketingCampaignEndpoints
{
    ...
    {
        var group = app.MapGroup("/api/content/campaigns")
            .WithTags("Marketing Campaigns")
        ...
        group.MapPost("/", async (CreateMarketingCampaignRequest request, ICommandDispatcher dispatcher,
                HttpContext context, CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(new CreateMarketingCampaignCommand(
                request.SiteKey, request.Name, request.Slug, request.Status, request.StartsAtUtc, request.EndsAtUtc,
                request.LandingContentId, request.UtmSource, request.UtmMedium, request.UtmCampaign,
                request.GoalType, request.SettingsJson, context.GetCurrentUserId()), ct), context))
            .RequireScope(ContentScopeNames.Edit);

        group.MapPut("/{id:guid}", async (Guid id, UpdateMarketingCampaignRequest request, ICommandDispatcher dispatcher,
                HttpContext context, CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(new UpdateMarketingCampaignCommand(
                id, request.SiteKey, request.Name, request.Slug, request.Status, request.StartsAtUtc, request.EndsAtUtc,
                request.LandingContentId, request.UtmSource, request.UtmMedium, request.UtmCampaign,
                request.GoalType, request.SettingsJson, context.GetCurrentUserId()), ct), context))
            .RequireScope(ContentScopeNames.Edit);
        ...
        group.MapDelete("/{id:guid}", async (Guid id, ICommandDispatcher dispatcher, HttpContext context, CancellationToken ct) =>
            EndpointResults.FromResult(await dispatcher.DispatchAsync(
                new DeleteMarketingCampaignCommand(id, context.GetCurrentUserId()), ct), context))
            .RequireScope(ContentScopeNames.Edit);
