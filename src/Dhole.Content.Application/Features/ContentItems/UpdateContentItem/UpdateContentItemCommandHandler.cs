...
{
    public async Task<Result> HandleAsync(UpdateContentItemCommand command, CancellationToken cancellationToken = default)
    {
        ...
        if (item.Type == ContentType.Page)
        {
            ...
            catch (ArgumentException) { return Result.Failure(ContentErrors.InvalidBlocksJson); }
        }

        NormalizedSeo seo;
        try
        {
            seo = SeoRules.Normalize(command.SeoTitle, command.SeoDescription, command.SeoKeywords,
                command.CanonicalUrl, command.Robots, command.OpenGraphMediaId, command.StructuredDataJson);
        }
        ...
        var slug = string.IsNullOrWhiteSpace(command.Slug) ? item.Slug : slugs.Generate(command.Slug);
        var locale = string.IsNullOrWhiteSpace(command.Locale) ? item.Locale : command.Locale.Trim();
        ...
        item.Update(command.Title, slug, command.Excerpt, blocksJson, command.RenderedHtml,
            command.FeaturedMediaId, command.SortOrder, command.IsFeatured, locale, command.UpdatedBy);
        item.ConfigureCmsMetadata(command.ParentContentId, command.TranslationGroupId, command.TemplateKey,
            command.UnpublishAtUtc, command.SitemapPriority, command.SitemapChangeFrequency, command.UpdatedBy);
        item.SetSeo(seo.Title, seo.Description, seo.Keywords, seo.CanonicalUrl, seo.Robots,
            seo.OpenGraphMediaId, seo.StructuredDataJson, command.UpdatedBy);

        if (command.TaxonomyTermIds is not null) item.ReplaceTaxonomies(command.TaxonomyTermIds);

        await audit.PublishAsync(new ContentAuditEvent(ContentAuditEventTypes.ContentUpdated, ContentAuditActions.Updated,
            ContentAuditEntityTypes.ContentItem, item.Id, command.UpdatedBy, Before: before,
