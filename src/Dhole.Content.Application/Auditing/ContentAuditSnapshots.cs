...
{
    public static object From(ContentItem x)=>new{x.Id,x.SiteKey,Type=x.Type.ToString(),Status=x.Status.ToString(),x.Title,x.Slug,x.Excerpt,x.BlocksJson,x.RenderedHtml,x.FeaturedMediaId,x.AuthorUserId,x.Locale,x.SortOrder,x.IsFeatured,x.SeoTitle,x.SeoDescription,x.SeoKeywords,x.CanonicalUrl,x.Robots,x.OpenGraphMediaId,x.StructuredDataJson,x.ScheduledAtUtc,x.PublishedAtUtc,x.IsDeleted,x.CreatedAtUtc,x.UpdatedAtUtc};
