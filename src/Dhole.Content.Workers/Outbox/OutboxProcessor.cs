...
namespace Dhole.Content.Workers.Outbox;

internal sealed class OutboxProcessor(ServiceDbContext dbContext, IRedisStreamPublisher redisStreamPublisher, IConfiguration configuration, ILogger<OutboxProcessor> logger) : IOutboxProcessor
{
    public async Task<OutboxProcessingResult> ProcessAsync(int batchSize, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var message in messages)
        {
            try
            {
                ...
                await redisStreamPublisher.PublishAsync(new RedisStreamMessage
                {
                    StreamName = ResolveStreamName(message.EventName),
            ...
            catch (Exception exception)
            {
                ...
                failedCount++;
                logger.LogError(exception, "Failed to publish Content outbox message {EventId}.", message.EventId);
            }
        }

        await dbContext.SaveChangesAsync(cancellationToken);
        var hasMoreMessages = await dbContext.OutboxMessages.AnyAsync(x => x.Status == OutboxMessageStatus.Pending, cancellationToken);
    ...
    private string ResolveStreamName(string eventName)
        => configuration[$"Redis:Streams:Destinations:{eventName}"] ?? configuration["Redis:Streams:DefaultStreamName"] ?? "dhole.content.events";
